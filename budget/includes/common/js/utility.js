...
if(typeof obj[i]!='function'){tm+=sep+i+":{"+obj[i]+"}\n";}else{}}}
return tm;}
if(typeof(obj)=="function")return sep+typeof(obj)+"\n";return sep+obj+"\n";}
...
return str;}
Object_weave_safe(utility,{dom:{}});utility.dom.setUnselectable=function(el){if(is.ie){utility.dom.attachEvent(el,'onselectstart',function(){var tagName=window.event.srcElement.tagName.toLowerCase();if(tagName=='input'||tagName=='textarea'){return true;}else{return false;}});}else{el.style.MozUserSelect='none';var inputs=el.getElementsByTagName('input');Array_each(inputs,function(input){utility.dom.attachEvent(input,'onfocus',function(){el.style.MozUserSelect='text';});utility.dom.attachEvent(input,'onblur',function(){el.style.MozUserSelect='none';});});}
var inputs=el.getElementsByTagName('img');Array_each(inputs,function(input){utility.dom.attachEvent(input,(is.ie?'ondragstart':'onmousedown'),function(e){if(is.mozilla){utility.popup.removeModal(e);}
utility.dom.stopEvent(e);return false;});});}
utility.dom.put=function(el,left,top){el.style.left=left+'px';el.style.top=top+'px';}
...
var EventCache=function(){var listEvents=[];return{listEvents:listEvents,add:function(node,sEventName,fHandler,bCapture){Array_push(listEvents,arguments);},flush:function(){var i,item;for(i=listEvents.length-1;i>=0;i=i-1){item=listEvents[i];if(item[0].removeEventListener){item[0].removeEventListener(item[1],item[2],item[3]);};var logical_type='';if(item[1].substring(0,2)!="on"){logical_type=item[1];item[1]="on"+item[1];}else{logical_type=item[1].substring(2,event_name_without_on.length);};if(typeof item[0].__eventHandlers!='undefined'&&typeof item[0].__eventHandlers[logical_type]!='undefined'){item[0].__eventHandlers[logical_type]=null;}
if(item[0].detachEvent){item[0].detachEvent(item[1],item[2]);};item[0][item[1]]=null;};listEvents=null;}};}();utility.dom.getStyleProperty=function(el,property){var value=el.style[property];if(!value){if(el.ownerDocument.defaultView&&typeof(el.ownerDocument.defaultView.getComputedStyle)=="function"){value=el.ownerDocument.defaultView.getComputedStyle(el,"").getPropertyValue(property);}else if(el.currentStyle){var m=property.split(/-/);if(m.length>0){property=m[0];for(var i=1;i<m.length;i++){property+=m[i].charAt(0).toUpperCase()+m[i].substring(1);}}
...
return r;}
utility.dom.setEventVars=function(e){var targ,relTarg,posx=0,posy=0;if(!e){e=window.event;}
if(!e){return{'e':null,'relTarg':null,'targ':null,'posx':0,'posy':0,'leftclick':false,'middleclick':false,'rightclick':false,'type':''};}
if(e.relatedTarget)relTarg=e.relatedTarget;else if(e.fromElement)relTarg=e.fromElement;if(e.target)targ=e.target;else if(e.srcElement)targ=e.srcElement;var st=utility.dom.getPageScroll();if(e.pageX||e.pageY){posx=e.pageX;posy=e.pageY;}else if(e.clientX||e.clientY){posx=e.clientX+st.x;posy=e.clientY+st.y;}
...
o={'e':e,'relTarg':relTarg,'targ':targ,'posx':posx,'posy':posy,'leftclick':leftclick,'middleclick':middleclick,'rightclick':rightclick,'type':e.type};return o;}
...
return o;}
Object_weave_safe(utility,{window:{}});utility.window.openWindow=function(target,url,width,height){var wndHandler;var left=(screen.width-width)/2;var top=(screen.height-height)/2;var winargs="width="+width+",height="+height+",resizable=No,scrollbars=No,status=Yes,modal=yes,dependent=yes,left="+left+",top="+top;var dialogargs="dialogWidth: "+(width+4)+"px; dialogHeight:"+(height+51)+"px; scroll:no";if(window.showModalDialog&&0){wndHandler=window.showModalDialog(url,window,dialogargs);}else{wndHandler=window.open(url,target,winargs);if(wndHandler){var ctrlModalBlocker=document.getElementById('modalBlocker');if(!ctrlModalBlocker){var ctrlModalBlocker=utility.dom.createElement("div",{'id':'modalBlocker','style':'display: block'});var pos=utility.dom.getPageInnerSize();ctrlModalBlocker.style.zIndex=1;ctrlModalBlocker.style.width=(pos.x)+'px';ctrlModalBlocker.style.height=(pos.y)+'px';prepfixieinsertnodescrollup();ctrlModalBlocker=document.body.insertBefore(ctrlModalBlocker,document.body.firstChild);fixieinsertnodescrollup();ctrlModalBlocker.onmousedown=utility.window.focusmodal;ctrlModalBlocker.onfocus=utility.window.focusmodal;utility.dom.attachEvent(window,'focus',utility.window.focusmodal);}else{ctrlModalBlocker.style.display='block';}
utility.window.reference=wndHandler;wndHandler.focus();}
if(!wndHandler){alert('Cannot open dialog. Please allow site popups.');}}
return wndHandler;}
function prepfixieinsertnodescrollup(){if(is.ie&&typeof(ktmls)!="undefined"){prepfixieinsertnodescrollup.scrolls=[];for(var i=0;i<ktmls.length;i++){prepfixieinsertnodescrollup.scrolls[i]=ktmls[i].edit.body.scrollTop;}}}
function fixieinsertnodescrollup(){window.setTimeout("fixieinsertnodescrollup_late()",1);}
function fixieinsertnodescrollup_late(){if(is.ie&&typeof(ktmls)!="undefined"){for(var i=ktmls.length-1;i>=0;i--){ktmls[i].edit.body.scrollTop=prepfixieinsertnodescrollup.scrolls[i];}}}
utility.window.focusmodal=function(){if(utility.window.reference&&!utility.window.reference.closed){utility.window.reference.focus();return;}
utility.window.hideModalBlocker();}
utility.window.hideModalBlocker=function(wnd){if(!wnd){wnd=window;}
utility.window.reference=null;var ctrlModalBlocker=wnd.document.getElementById('modalBlocker');if(ctrlModalBlocker){ctrlModalBlocker.style.display='none';}}
utility.window.close=function(){window.close();}
utility.popup={};utility.popup.stiva=[];utility.popup.makeModal=function(clickCallBack,elementOnTop,stopEvents){if(typeof(stopEvents)=="undefined"){stopEvents=true;}
utility.popup.stiva.push({'element':elementOnTop,'callback':clickCallBack,'stopEvents':stopEvents});}
utility.popup.removeModal=function(e){if(utility.popup.stiva.length==0){return;}
...
utility.popup.escapeModal=function(e){if(utility.popup.stiva.length>0){if(!utility.popup.stiva[utility.popup.stiva.length-1].stopEvents){return true;}
...
else{pos=utility.dom.getBox(el);ctrlInterfaceBlocker.style.top=pos.y+'px';ctrlInterfaceBlocker.style.left=pos.x+'px';ctrlInterfaceBlocker.style.width=pos.width+'px';ctrlInterfaceBlocker.style.height=pos.height+'px';}}
utility.window.unblockInterface=function(){var ctrlInterfaceBlocker=document.getElementById('interfaceBlocker');if(ctrlInterfaceBlocker){document.body.removeChild(ctrlInterfaceBlocker);}}
utility.window.setModal=function(){if(!window.dialogArguments){window.onbeforeunload=function(){utility.window.hideModalBlocker(window.opener);}
utility.dom.setUnselectable(window.document.body);}else{window.opener=dialogArguments;}
if(window.opener.topOpener){window.topOpener=window.opener.topOpener;}else{window.topOpener=window.opener;}
utility.dom.attachEvent(is.ie?window.document.body:window,'keydown',function(e){var ret=utility.popup.escapeModal(e);if(ret&&e.keyCode==27){utility.window.close();}});utility.dom.attachEvent2(window.document.body,'mousedown',utility.popup.removeModal);}
Object_weave_safe(utility,{cookie:{}});utility.cookie.set=function(name,value,lifespan,access_path){var cookietext=name+"="+escape(value);if(lifespan!=null){var date=new Date();date.setTime(date.getTime()+(1000*60*60*24*lifespan));cookietext+="; expires="+date.toGMTString();}
...
function sortFormHandlers(arr){for(var i=0;i<arr.length;i++){var fh1=arr[i];for(var j=i+1;j<arr.length;j++){var fh2=arr[j];if(fh2[0]<fh1[0]){var tmp=fh1;arr[i]=fh2;arr[j]=tmp;}}}}
function GLOBAL_registerFormSubmitEventHandler(function_name,priority){var frms=document.getElementsByTagName('form');for(var i=0;i<frms.length;i++){var frm=frms[i];if(typeof frm.onsubmit!='undefined'&&frm.onsubmit!=null){var form_handlers=frm.form_handlers;if(form_handlers){form_handlers[form_handlers.length]=[priority,function_name];sortFormHandlers(form_handlers);}else{var tmp=frm.onsubmit.toString();var str=tmp.substring(tmp.indexOf('{')+1,tmp.lastIndexOf('}'));frm.onsubmit=new Function('e','if (!KT_formSubmittalHandler(e)) return false; '+str);form_handlers=[];form_handlers[form_handlers.length]=[priority,function_name];}
frm.form_handlers=form_handlers;}else{frm.onsubmit=new Function('e','return KT_formSubmittalHandler(e);');var form_handlers=[];form_handlers[form_handlers.length]=[priority,function_name];frm.form_handlers=form_handlers;}}}
var global_form_submit_lock=false;function KT_formSubmittalHandler(e){var o=utility.dom.setEventVars(e);if(global_form_submit_lock){utility.dom.stopEvent(e);return false;}
var frm=o.targ;if(!frm){return true;}
if(!frm.tagName){return true;}
if(frm.tagName.toLowerCase()!="form"){frm=frm.form;}
if(!frm){return true;}
if(typeof(UNI_disableButtons)!='undefined'){UNI_disableButtons(frm,/.*/,true);}
var ret=true;var form_handlers=frm.form_handlers;if(form_handlers){for(var i=0;i<form_handlers.length;i++){var fun=form_handlers[i];eval("ret = "+fun[1]+"(e);");if(!ret){break;}}}
if(is.ie&&is.mac){UNI_disableButtons(frm,/.*/,false);}
if(!ret){if(!global_form_submit_lock&&typeof(UNI_disableButtons)!='undefined'){UNI_disableButtons(frm,/.*/,false);}
