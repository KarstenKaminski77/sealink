...
function csstree_click(e) {
	...
	if (utility.dom.getClassNames(li).indexOf('current') == -1) {
...
function csstree_attach(menu, parentObj, path, lev) {
	...
	lis.each(function(li, i) {
		...
		var issel = false;
		if (
			utility.dom.getClassNames(li).indexOf('selected') != -1 || 
		...
		if (utility.dom.getClassNames(li).indexOf('selected') != -1) {
		...
		['ondblclick','onselect','onselectstart','ondragstart'].each(function(ev){
			utility.dom.attachEvent(sign, ev, function(e){
				var o = utility.dom.setEventVars(e);
				return utility.dom.stopEvent(o.e);
			});
		});
		if (['', '#', window.location.href, window.location + '#'].indexOf(a.href) != -1) {
...
function csstree_bind_all() {
	...
	utility.dom.getElementsByClassName(document, $KT_CSSTREE_DEFAULTS.classname, 'div').each(function(div, i){
