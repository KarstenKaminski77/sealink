...
if(isset($_GET['Preview'])){
	...
	$document = 'Seavest JSA Elevated #'.$id.'.pdf';
	
	$pdf->Output('pdf/'. $document);
	
	mysql_query("UPDATE tbl_hes_documents_relation SET PDF = '$document', Active = '1', New = '0' WHERE HESId = '$id' AND DocumentId = '7'")or die(mysql_error());
	...
	mysql_query("UPDATE tbl_hes_documents_relation SET PDF = '$document' WHERE HESId = '$id' AND DocumentId = '7'")or die(mysql_error());
	
	header('Location: ../health-safety/jsa/jsa-elevated.php?Id='. $_GET['Id']);
}
