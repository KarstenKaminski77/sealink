...
if(!isset($_POST['search-2'])){
	...
	$where .= " AND tbl_scheduled_maintenance.Status = 'In Progress'";
}
...
<html xmlns="http://www.w3.org/1999/xhtml">
   ...
   <body>
      ...
      <?php } ?>
        ...
        <div id="list-border">
          <form id="form2" name="form2" method="post" action="list.php">
            <table width="100%" border="0" align="center" cellpadding="3" cellspacing="1">
              <tr>
                ...
                <td class="td-header" width="120" align="left"><strong>Date</strong></td>
                <td width="120" colspan="2" align="left" class="td-header">Status</td>
              </tr>
              <tr>
              ...
              <tr class="<?php echo ($ac_sw1++%2==0)?" odd ":"even "; ?>" onmouseover="this.oldClassName = this.className; this.className='over';" onmouseout="this.className = this.oldClassName;">
                <td width="100"><?php echo $row_list[ 'Name']; ?></td>
                <td width="200"><?php echo $row_list[ 'Description']; ?></td>
                <td><?php echo $row_list[ 'Quarter']; ?></td>
                <td width="150">
				
                <input name="date-list<?php echo $i; ?>" class="tarea-100" id="date-list<?php echo $i; ?>" value="<?php echo $row_list[ 'Date']; ?>" />
                <script type="text/javascript">
                          $('#date-list<?php echo $i; ?>').datepicker({
                          dateFormat: "yy-mm-dd"
                          });
                          </script>
				...
				// Post search parameters for first status change
				if(!empty($_POST['name'])){
					
					$param .= 'Site='. $_POST['name'] .'&';
				}
				
				if(!empty($_POST['type'])){
					
					$param .= 'Description='. $_POST['type'] .'&';
				}
				
				if(!empty($_POST['quarter'])){
					
					$param .= 'Quarter='. $_POST['quarter'] .'&';
				}
				
				// Get parameter for asecond or more status changes
				if(!empty($_GET['Site'])){
					
					$param .= 'Site='. $_GET['Site'] .'&';
				}
				...
				if(!empty($_GET['Quarter'])){
					...
					$param .= 'Quarter='. $_GET['Quarter'] .'&';
				}
				
				?>
                
                <select name="jumpMenu" class="tarea-100" id="jumpMenu" onchange="MM_jumpMenu('parent',this,0)">
                    <option value="list.php?<?php echo $param; ?>Id=<?php echo $row_list['Id']; ?>&Status=Qued" 
					<?php if($row_list['Status'] == 'Qued'){ echo 'selected="selected"'; } ?>>Qued</option>
                    <option value="list.php?<?php echo $param; ?>Id=<?php echo $row_list['Id']; ?>&Status=In Progress" 
					<?php if($row_list['Status'] == 'In Progress'){ echo 'selected="selected"'; } ?>>In Progress</option>
                    <option value="list.php?<?php echo $param; ?>Id=<?php echo $row_list['Id']; ?>&Status=Complete" 
					<?php if($row_list['Status'] == 'Complete'){ echo 'selected="selected"'; } ?>>Complete</option>
                </select>
                </td>
                <?php
				...
				?>
                <td width="20" align="center"><a href="events.php?<?php echo 'Day='. $day .'&Month='. $month .'&Year='. $year; ?>" class="calendar various3"></a></td>
              </tr>
