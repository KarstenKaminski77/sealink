...
$query_Recordset2 = sprintf("SELECT tbl_orders.OrderNo, tbl_suppliers.Name, tbl_suppliers.Address, tbl_order_details.Qty, tbl_order_details.Description, tbl_order_details.Amount, tbl_orders.TechnicianId, tbl_orders.SiteId, tbl_orders.JobNo, tbl_orders.Issuer, tbl_order_relation.OrderId FROM (((tbl_orders LEFT JOIN tbl_order_relation ON tbl_order_relation.OrderId=tbl_orders.Id) LEFT JOIN tbl_order_details ON tbl_order_details.Id=tbl_order_relation.ItemId) LEFT JOIN tbl_suppliers ON tbl_suppliers.Id=tbl_orders.SupplierId) WHERE tbl_order_relation.OrderId=%s ", GetSQLValueString($KTColParam1_Recordset2, "int"));
