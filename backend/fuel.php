...
// Filter
$tfi_listtbl_fuel1 = new TFI_TableFilter($conn_seavest, "tfi_listtbl_fuel1");
$tfi_listtbl_fuel1->addColumn("tbl_companies.Name", "STRING_TYPE", "Name", "%");
...
// Navigation
$nav_listtbl_fuel1 = new NAV_Regular("nav_listtbl_fuel1", "rstbl_fuel1", "../", $_SERVER['PHP_SELF'], 20);
...
$query_rstbl_fuel1 = sprintf("SELECT tbl_companies.Name, tbl_fuel.Rate, tbl_fuel.Id FROM tbl_fuel LEFT JOIN tbl_companies ON tbl_fuel.Company = tbl_companies.Id WHERE %s ORDER BY %s", $NXTFilter_rstbl_fuel1, $NXTSort_rstbl_fuel1);
$query_limit_rstbl_fuel1 = sprintf("%s LIMIT %d, %d", $query_rstbl_fuel1, $startRow_rstbl_fuel1, $maxRows_rstbl_fuel1);
$rstbl_fuel1 = mysql_query($query_limit_rstbl_fuel1, $seavest) or die(mysql_error());
...
$ins_tbl_fuel = new tNG_multipleInsert($conn_seavest);
...
$ins_tbl_fuel->addColumn("Company", "STRING_TYPE", "POST", "Company");
$ins_tbl_fuel->addColumn("Rate", "DOUBLE_TYPE", "POST", "Rate");
$ins_tbl_fuel->setPrimaryKey("Id", "NUMERIC_TYPE");

// Make an update transaction instance
$upd_tbl_fuel = new tNG_multipleUpdate($conn_seavest);
            ...
            <table cellpadding="2" cellspacing="0" class="KT_tngtable">
              <thead>
                <tr class="KT_row_order">
                  ...
                  <th id="Name" class="KT_sorter KT_col_Name <?php echo $tso_listtbl_fuel1->getSortIcon('tbl_companies.Name'); ?>"> <a href="<?php echo $tso_listtbl_fuel1->getSortLink('tbl_companies.Name'); ?>">Company</a> </th>
...
?>
                  <tr class="KT_row_filter">
                    <td>&nbsp;</td>
                    <td><input type="text" name="tfi_listtbl_fuel1_Name" id="tfi_listtbl_fuel1_Name" value="<?php echo KT_escapeAttribute(@$_SESSION['tfi_listtbl_fuel1_Name']); ?>" size="20" maxlength="100" /></td>
...
?>
                  ...
                  <?php do { ?>
                    <tr class="<?php echo @$cnt2++%2==0 ? "" : "KT_even"; ?>">
                      <td><input type="checkbox" name="kt_pk_tbl_fuel" class="id_checkbox" value="<?php echo $row_rstbl_fuel1['Id']; ?>" />
                          <input type="hidden" name="Id" class="id_field" value="<?php echo $row_rstbl_fuel1['Id']; ?>" />
                      </td>
                      <td><div class="KT_col_Name"><?php echo KT_FormatForList($row_rstbl_fuel1['Name'], 20); ?></div></td>
                      <td><div class="KT_col_Rate"><?php echo KT_FormatForList($row_rstbl_fuel1['Rate'], 20); ?></div></td>
                      <td><a class="KT_edit_link" href="fuel.php?Id=<?php echo $row_rstbl_fuel1['Id']; ?>&amp;KT_back=1"><?php echo NXT_getResource("edit_one"); ?></a> <a class="KT_delete_link" href="#delete"><?php echo NXT_getResource("delete_one"); ?></a> </td>
                    </tr>
          ...
          ?>
            ...
            <div class="KT_bottombuttons">
              ...
              <a class="KT_additem_op_link" href="fuel.php?KT_back=1" onclick="return nxt_list_additem(this)"><?php echo NXT_getResource("add new"); ?></a> </div>
          </form>
        </div>
        <br class="clearfixplain" />
      </div>
