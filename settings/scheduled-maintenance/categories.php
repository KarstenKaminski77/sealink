...
if(isset($_POST['reset'])){
   ...
   $edit = $_GET['Edit'];
	...
	if(isset($_POST['insert'])){
		...
		dbInsert('tbl_sm_cat', $form_data,$con);
	...
	if(isset($_POST['update'])){
		...
		dbUpdate('tbl_sm_cat', $form_data, $where_clause="Id = '". $_GET['Edit'] ."'",$con);
	...
	if(isset($_GET['Delete'])){
		...
		dbDelete('tbl_sm_cat', $where_clause="Id = '". $_GET['Delete'] ."'",$con);
	...
	$query_list = "
		SELECT
			tbl_sm_cat.Id,
			tbl_sm_cat.CompanyId,
			tbl_sm_cat.Cat,
			tbl_companies.`Name`
		FROM
			tbl_sm_cat
		INNER JOIN tbl_companies ON tbl_companies.Id = tbl_sm_cat.CompanyId
		ORDER BY tbl_sm_cat.Cat ASC";
	
	$query_list = mysqli_query($con, $query_list)or die(mysqli_error($con));
	$numrows = mysqli_num_rows($query_list);
		
	$query_form = mysqli_query($con, "SELECT * FROM tbl_sm_cat WHERE Id = '$edit'")or die(mysqli_error($con));
                  ...
                  <td class="td-right">
                    <select name="company" class="tarea-100" id="company">
                      ...
                      <?php while($row_companies = mysqli_fetch_array($query_companies)){ ?>
                        <option value="<?php echo $row_companies['Id']; ?>" <?php if($row_form['CompanyId'] == $row_companies['Id']){ echo 'selected="selected"'; } ?>><?php echo $row_companies['Name']; ?></option>
                      <?php } ?>
                    </select>
                  </td>
                </tr>
                <tr>
                  <td width="130" class="td-left">Category</td>
                  <td class="td-right"><input name="cat" type="text" class="tarea-100" id="cat" value="<?php echo $row_form['Cat']; ?>" /></td>
                </tr>
              </table>
            </div>
          </td>
          ...
          <td colspan="2">
            <div id="list-border">
              <table width="100%" border="0" cellpadding="4" cellspacing="1">
                <tr>
                  <td width="414" class="td-header">Categories</td>
                  <td width="413" class="td-header">Oil Company</td>
                ...
                <tr class="<?php echo ($ac_sw1++%2==0)?" even ":"odd "; ?>" onmouseover="this.oldClassName = this.className; this.className='over';" onmouseout="this.className = this.oldClassName;">
                  ...
                  <td align="center">
                    <a href="categories.php?Delete=<?php echo $row_list['Id']; ?>" title="Delete" class="delete" onclick = "if (!confirm('Delete <?php echo $row_list['Cat']; ?>?')){ return false; }"></a>
                  </td>
                  <td align="center">
                    <a href="categories.php?Edit=<?php echo $row_list['Id']; ?>" title="Edit" class="edit"></a>
                  </td>
                </tr>
                <?php } ?>
              </table>
