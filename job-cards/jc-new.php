...
if (isset($_POST['Submit']) && empty($errors)) {
  ...
  if ($sla_rows >= 1 || (!empty($_POST['start']) && !empty($_POST['end']))) {
    ...
    $sla_cat = $_POST['sla'];
    $sla_sub_cat = ($_POST['sub_cat'] != '') ? $_POST['sub_cat'] : 0;
    $contractor = ($_POST['contractor'] != '') ? $_POST['contractor'] : 0;
    ...
    if ($numrows >= 1) {
      ...
      $sql = "INSERT INTO `tbl_jc` (`AreaId`, `Status`, `JobId`, `JobNo`, `CompanyId`, `SiteId`, `SlaCatId`, `SlaSubCatId`, `Unit`, `Total2`, `Date`, `Labour`, `JobDescription`, `JobDescriptionOperator`, `JobDescriptionDate`, `Date1`, `Date2`, `NewInvoiceDate`, `Reference`, `CustomerFeedBack`, `Days`, `Import`, `Archive`, `CostingHint`, `HistoryPlatform`, `JobcardPDF`, `Progress`, `JcDate`, `LabourType`, `SQLdate`, `SLAComments`, `SLAClose`, `ContractorId`, `SystemId`) VALUES ($areaid, 1, $jobid, '$jobno', $company, $site, $sla_cat, $sla_sub_cat, 'hours', 0.00, '$date', 1, '". mysqli_escape_string($con, $job_desc) ."', '" .$_COOKIE['name'] ."', '$jcdate', '$start', '$end', '0000-00-00', '$ref', '', '$today', 0, 0, '', 0, '', 0, '$jcdate', '', '0000-00-00', '', 0, $contractor, $systemid)";

      mysqli_query($con, $sql) or die(mysqli_error($con));

      $sql = "INSERT INTO `tbl_jc` (`AreaId`, `Status`, `JobId`, `JobNo`, `CompanyId`, `SiteId`, `SlaCatId`, `SlaSubCatId`, `Total2`, `Date`, `Material`, `JobDescription`, `JobDescriptionOperator`, `JobDescriptionDate`, `Date1`, `Date2`, `NewInvoiceDate`, `Reference`, `CustomerFeedBack`, `Days`, `Import`, `Archive`, `CostingHint`, `HistoryPlatform`, `JobcardPDF`, `Progress`, `JcDate`, `LabourType`, `SQLdate`, `SLAComments`, `SLAClose`, `ContractorId`, `SystemId`) VALUES ($areaid, 1, $jobid, '$jobno', $company, $site, $sla_cat, $sla_sub_cat, 0.00, '$date', 1, '". mysqli_escape_string($con, $job_desc) ."', '" .$_COOKIE['name'] ."', '$jcdate', '$start', '$end', '0000-00-00', '$ref', '', '$today', 0, 0, '', 0, '', 0, '$jcdate', '', '0000-00-00', '', 0, $contractor, $systemid)";

      mysqli_query($con, $sql) or die(mysqli_error($con));

      mysqli_query($con, "INSERT INTO tbl_travel (JobId) VALUES ('$jobid')") or die(mysqli_error($con));

      $sql = "INSERT INTO `tbl_jc` (`AreaId`, `Status`, `JobId`, `JobNo`, `CompanyId`, `SiteId`, `SlaCatId`, `SlaSubCatId`, `Total2`, `Date`, `Comment`, `JobDescription`, `JobDescriptionOperator`, `JobDescriptionDate`, `Date1`, `Date2`, `NewInvoiceDate`, `Reference`, `CustomerFeedBack`, `Days`, `Import`, `Archive`, `CostingHint`, `HistoryPlatform`, `JobcardPDF`, `Progress`, `JcDate`, `LabourType`, `SQLdate`, `SLAComments`, `SLAClose`, `ContractorId`, `SystemId`) VALUES ($areaid, 1, $jobid, '$jobno', $company, $site, $sla_cat, $sla_sub_cat, 0.00, '$date', 1, '". mysqli_escape_string($con, $job_desc) ."', '" .$_COOKIE['name'] ."', '$jcdate', '$start', '$end', '0000-00-00', '$ref', '', '$today', 0, 0, '', 0, '', 0, '$jcdate', '', '0000-00-00', '', 0, $contractor, $systemid)";
...
//END GET ALL CATEORIES

//START GET ALL COMPANIES
$results = [];
$companyQuery = "SELECT * FROM tbl_companies ORDER BY Name ASC";
  ...
  <div id="main-wrapper">
    ...
    <?php if ($_SESSION['areaid'] != 0) { ?>
      <form action="" method="post" enctype="multipart/form-data" name="f1" class="uk-form bt-flabels js-flabels" data-parsley-validate data-parsley-errors-messages-disabled>
        ...
        <div id="list-border" style="margin-bottom:20px">
          <table width="100%" border="0" align="center" cellpadding="2" cellspacing="1">
            ...
            <tr>
              ...
              <td width="256" nowrap class="td-right">
                ...
                <div class="bt-flabels__wrapper">
                  <select name="company" id="country-list" class="tarea-100" onChange="getSites(this.value);" autocomplete="off" data-parsley-required>
                    <option value="">Oil Company</option>
                    <?php foreach ($results as $company) { ?>
                      <option value="<?php echo $company["Id"]; ?>" <?php if ($company["Id"] == $_POST['company']) {
            ...
            <tr>
              ...
              <td class="td-right">
                <div class="bt-flabels__wrapper">
                  <input name="jobnumber" type="text" class="tarea-100" id="jobnumber" value="<?php echo $_GET['JobNo']; ?>" autocomplete="off" data-parsley-required>
              ...
              <td width="256" class="td-right">
                ...
                <div class="bt-flabels__wrapper">
                  <select name="reference" class="tarea-100" id="reference" autocomplete="off" data-parsley-required>
                    <option value="">Select an Engineer</option>
                    <?php while ($row_engineers = mysqli_fetch_array($query_engineers)) { ?>
                      <option value="<?php echo trim($row_engineers['Name']); ?>" <?php if ($row_engineers['Name'] == $_GET['Reference']) {
                                                                                    echo 'selected="selected"';
                                                                                  } ?>>
                        <?php echo $row_engineers['Name']; ?></option>
                    <?php } ?>
                  </select>
                  <span class="bt-flabels__error-desc-dd">Required</span>
                </div>
              ...
              <td class="td-right">
                ...
                <select name="contractor" class="tarea-100" id="contractor">
                  ...
                  <?php while ($row_contractor = mysqli_fetch_array($query_contractor)) { ?>
                    <option value="<?php echo $row_contractor['Id']; ?>" <?php if ($_GET['Contractor'] == $row_contractor['Id']) {
                                                                            echo 'selected="selected"';
                                                                          } ?>><?php echo $row_contractor['Name']; ?>
                    </option>
                  <?php } ?>
                </select>

              </td>
              <td class="td-left">Upload JC (PDF)</td>
              <td class="td-right">

                <div class="bt-flabels__wrapper">
                  <input name="pdf" type="file" class="tarea-100" id="pdf" size="27" autocomplete="off" data-parsley-required />
                  <span class="bt-flabels__error-desc">Required</span>
                </div>
            ...
            <tr>
              ...
              <td colspan="3" class="td-right">
                ...
                <div class="bt-flabels__wrapper">
                  <textarea name="description" cols="45" rows="5" class="tarea-100" id="description" autocomplete="off" data-parsley-trigger="keyup" data-parsley-minlength="9" data-parsley-required><?php echo $_GET['Description']; ?></textarea>
                  <span class="bt-flabels__error-desc">Minimum Of 5 Words Required</span>
                </div>
              </td>
            </tr>
            <tr>
              <td valign="top" class="td-left">Category</td>
              <td colspan="3" class="td-right">
                <div class="bt-flabels__wrapper">
                  <select name="system" class="tarea-100" data-parsley-required>
                    <option value="">Select one...</option>
                    <?php while ($row_system = mysqli_fetch_array($query_system)) { ?>
                      <option value="<?php echo $row_system['Id']; ?>"><?php echo $row_system['Name']; ?>
                      </option>
                    <?php } ?>
                  </select>
                  <span class="bt-flabels__error-desc-dd">Required</span>
        ...
        <div id="list-border" style="margin-bottom:20px">
          <table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" id="sla-tbl">
            <tr>
              <td colspan="4" class="td-header">Description</td>
            </tr>
            <tr id="tbl-row-1">
              <td width="140" class="td-left">Type</td>
                ...
                <div class="bt-flabels__wrapper">
                  <select name="sla" class="tarea-100" id="sla" onChange="getSlaSub(this.value);" autocomplete="off" data-parsley-required>
                    ...
                    <?php while ($row_sla_cat = mysqli_fetch_array($query_sla_cat)) { ?>
                      <option value="<?php echo $row_sla_cat['Id']; ?>" <?php if ($_GET['SLA'] == $row_sla_cat['Id']) {
                                                                          echo 'selected="selected"';
              ...
              <td width="140" class="td-left" id="col-3" style="display:table-cell">Work Type</td>
              <td class="td-right" id="col-4" style="display:table-cell">
                ...
                <div class="bt-flabels__wrapper">
                  ...
                  <select name="sub_cat" class="tarea-100" id="sla-list" autocomplete="off">
            ...
            <tr id="sla-date">
              <td width="140" class="td-left">Start Date</td>
              <td class="td-right">
          ...
          <table width="100%" border="0" align="center" cellpadding="2" cellspacing="1">
            ...
            <tr>
              ...
              <td width="256" class="td-right">
                ...
                <div class="bt-flabels__wrapper">
                  <select name="risk" class="tarea-100" id="risk" style="width:100%" autocomplete="off" data-parsley-required>
                    <option value="">Select one...</option>
                    <?php
                    do {
                    ?>
                      <option value="<?php echo $row_Recordset4['Id']; ?>" <?php if ($row_Recordset4['Id'] == $_POST['risk']) {
                                                                              echo 'selected="selected"';
                                                                            } ?>><?php echo $row_Recordset4['Risk'] ?>
                      </option>
                    <?php
              ...
              <td width="256" class="td-right"><select name="type" class="tarea-100" id="type" style="width:100%">
                  ...
                  do {
                  ?>
                    <option value="<?php echo $row_Recordset3['Id']; ?>" <?php if ($row_Recordset3['Id'] == $_POST['type']) {
                                                                            echo 'selected="selected"';
                                                                          } ?>><?php echo $row_Recordset3['Risk'] ?>
                    </option>
                  <?php
