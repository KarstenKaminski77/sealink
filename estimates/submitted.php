...
$query_Recordset3 = "
	SELECT
	  tbl_qs.QuoteNo AS QuoteNo_1,
	  tbl_qs.AreaId,
	  tbl_qs.JobDescription,
	  tbl_sent_quotes.QuoteNo,
	  tbl_sent_quotes.CompanyId,
	  tbl_sent_quotes.SiteId,
	  tbl_sent_quotes.PDF,
	  tbl_sent_quotes.DateSent,
	  tbl_sent_quotes.Sent
	FROM
	  (
		tbl_sent_quotes
		LEFT JOIN tbl_qs
		  ON tbl_qs.QuoteNo = tbl_sent_quotes.QuoteNo
	  )
	WHERE STATUS = '2'
	$where
	$sql_where
	GROUP BY QuoteNo
	ORDER BY tbl_qs.Days ASC";
...
<html xmlns="http://www.w3.org/1999/xhtml">
   ...
   <body>
      ...
      <!-- Main Form -->
      <div id="main-wrapper">
      <form action="../fpdf16/pdf/quote_resend_mail.php" method="post" enctype="multipart/form-data" name="form2">
        <div id="list-border">
         ...
         <div id="list-border">
           <table width="100%" border="0" align="center" cellpadding="3" cellspacing="1">
             <tr>
               ...
               <td width="120" class="td-header"><strong>Date</strong></td>
               <td width="80" class="td-header">Age</td>
               <td width="100" class="td-header">Flw. Up</td>
               <td width="40" align="center" class="td-header-right"></td>
               <td width="40" align="center" class="td-header-right"></td>
               <td width="40" align="center" class="td-header-right"></td>
               <td width="40" align="center" class="td-header-right"></td>
               <td width="40" align="center" class="td-header-right"></td>
...
?>
             <tr class="<?php echo ($ac_sw1++%2==0)?"odd":"even"; ?>" onmouseover="this.oldClassName = this.className; this.className='over';" onmouseout="this.className = this.oldClassName;">
               ...
               <td width="250"><div><?php echo date('Y-m-d', strtotime($row_Recordset3['DateSent'])); ?></div></td>
               <td width="80"><?php
					$quoteno = $row_Recordset3['QuoteNo'];
					time_schedule_quotes($quoteno); ?></td>
               <td nowrap="nowrap"><?php follow_up($quoteno); ?></td>
               <td align="center" style="padding:0"><a href="../fpdf16/pdf_quotation.php?Id=<?php echo $row_Recordset3['QuoteNo']; ?>&amp;preview" target="_blank" class="view view-pdf"></a></td>
               <td align="center" style="padding:0"><a href="revive.php?Id=<?php echo $row_Recordset3['QuoteNo']; ?>" class="edit" onclick="return confirmSubmit()"></a></td>
               <td align="center" style="padding:0">
                 <a href="history.php?Id=<?php echo $row_Recordset3['QuoteNo']; ?>" class="icon-info fancybox" title="View History"></a>
               </td>
               <td align="center" style="padding:0"><a href="photos.php?Id=<?php echo $row_Recordset3['QuoteNo_1']; ?>" class="image-icon view-photos"></a></td>
               <td align="center" style="padding:0"><a href="update-status.php?Id=<?php echo $row_Recordset3['QuoteNo']; ?>" class="archives fancybox"></a></td>
               <td align="center" style="padding:0"><input name="file[]" type="radio" id="file[]" value="<?php echo $row_Recordset3['PDF']; ?>" /></td>
             </tr>
             <?php
                ...
                $row_Recordset3['SiteId'] = NULL;
                $row_Recordset3['DateSent'] = NULL;
                $quoteno = NULL;
                $row_Recordset3['QuoteNo_1'] = NULL;
