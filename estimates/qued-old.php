...
mysql_select_db($database_seavest, $seavest);
$query_Recordset3 = "SELECT tbl_sites.Name AS Name_1, tbl_users.Name AS Username, tbl_qs.QuoteNo, tbl_companies.Name, tbl_companies.Id AS CompanyId, tbl_qs.Date, tbl_qs.Time, tbl_qs.JobDescription, tbl_qs.Id, tbl_qs.Rejected FROM (((tbl_qs LEFT JOIN tbl_companies ON tbl_companies.Id=tbl_qs.CompanyId) LEFT JOIN tbl_sites ON tbl_sites.Id=tbl_qs.SiteId) LEFT JOIN tbl_users ON tbl_users.Id=tbl_qs.UserId) WHERE Status = '4' $where GROUP BY QuoteNo ORDER BY tbl_qs.Date ASC";
...
<tr class="<?php echo ($ac_sw1++%2==0)?"odd":"even"; ?>" onMouseOver="this.oldClassName = this.className; this.className='list-over';" onMouseOut="this.className = this.oldClassName;" <?php echo $style; ?>>
                          ...
                          <td width="80" nowrap>
						  <a href="../quote_calc.php?Id=<?php echo $row_Recordset3['QuoteNo']; ?>&job" class="menu" <?php echo $style; ?>>
						  <?php echo $row_Recordset3['Username']; ?>
                          </a>
                          </td>
                          </tr>
