...
if(!isset($_COOKIE['userid']) || !isset($_COOKIE['areaid']) || !isset($_COOKIE['name'])){
...
if($row_qs['Status'] == 0 && !isset($_COOKIE['costing-'. $_GET['Id']])){
...
if(isset($_POST['desc-hes']) && isset($_COOKIE['costing-'. $_GET['Id']])){
	...
	dbUpdate('tbl_qs_hes', $form_data, $where_clause="QuoteNo = '". $_GET['Id'] ."'",$con);
	...
	dbDelete('tbl_qs_hes', $where_clause="QuoteNo = '". $quoteno ."' AND Old = '1'", $con);
	...
	dbUpdate('tbl_qs_equipment', $form_data, $where_clause="QuoteNo = '". $_GET['Id'] ."'",$con);
	...
	dbDelete('tbl_qs_equipment', $where_clause="QuoteNo = '". $quoteno ."' AND Old = '1'", $con);


	$query = mysqli_query($con, "SELECT * FROM tbl_qs WHERE QuoteNo = '$quoteno'") or die(mysqli_error($con));
	$row = mysqli_fetch_array($query);

	$query_area = mysqli_query($con, "SELECT AreaId FROM tbl_sites WHERE Id = '". $row['SiteId'] ."'")or die(mysqli_error($con));
	$row_area = mysqli_fetch_array($query_area);

	mysqli_query($con, "UPDATE tbl_qs SET Temp = '1' WHERE QuoteNo = '$quoteno'")or die(mysqli_error($con));
	...
	for($i=0;$i<count($_POST['transport']);$i++){
		...
		$form_data = array(
			...
			'Total1' => $_POST['transport'][$i] * $_POST['qty_t'][$i] * $_POST['price_t'][$i],
			'Email' => $row['Email'],
			'Date' => $_POST['date'],
			'SlaStart' => $row['SlaStart'],
			'SlaEnd' => $row['SlaEnd'],
	...
	dbDelete('tbl_qs', $where_clause="QuoteNo = '". $quoteno ."' AND Temp = '1'", $con);
}
...
if(isset($_POST['notes'])){
	...
	dbUpdate('tbl_qs', $misc_data, $where_clause="QuoteNo = '". $_GET['Id'] ."'",$con);
}
...
if(isset($_POST['jobcard']) && !empty($_POST['jobno'])){
	...
	while($row = mysqli_fetch_array($query)){
		...
		mysqli_query($con, "INSERT INTO tbl_jc (AreaId,QuoteNo,SlaCatId,SlaSubCatId,JobNo,CompanyId,SiteId,Description,Unit,Qty,
		Price,Total1,Date,Labour,Material,Transport,JobId,TransportComment,JC,Status,SystemId)
		VALUES
		('$areaid','$quote','$sla_cat','$sla_sub_cat','$jobno','$companyid','$siteid','$desc','$unit','$qty','$price','$total','$date','$labour','$material','$transport','$jobid','$t_comment','1','1',$systemid)") or die(mysqli_error($con));
	}

	header('Location: ../job-cards/qued-details.php?Id='. $jobid .'');
...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  $(document).ready(function() {
   ...
   <body>
      ...
      <!-- Main Form -->
      <div id="main-wrapper">
      <form id="form2" name="form2" method="post" action="quote-calc.php?Id=<?php echo $quoteno; ?>&update" class="uk-form bt-flabels js-flabels" data-parsley-validate data-parsley-errors-messages-disabled>
      ...
      <!-- Quote To Details -->
       <div id="list-border">
         <table width="100%" border="0" align="center" cellpadding="0" cellspacing="1">
           ...
           <tr>
             ...
             <td colspan="2" class="td-right">
               ...
               <div class="bt-flabels__wrapper">
                 <input name="date" class="tarea-100" id="date" value="<?php echo $row_Recordset5['Date']; ?>" autocomplete="off" data-parsley-required />
                   <script type="text/javascript">
                        $('#date').datepicker({
                        dateFormat: "yy-mm-dd"
                        });
           ...
           <tr>
             <td rowspan="3" valign="top" class="td-left">Address</td>
             <td rowspan="3" valign="top" class="td-right" style="padding-bottom:0px;"><?php echo nl2br($row_Recordset3['Address']); ?></td>
             <td rowspan="2" valign="top" class="td-left">Site Address</td>
             <td rowspan="2" valign="top" class="td-right"><?php echo char_limit($row_Recordset5['Address'], 30); ?></td>
             <td class="td-left">Quote No.</td>
             <td colspan="2" class="td-right"><?php echo $row_Recordset5['QuoteNo']; ?></td>
           </tr>
           <tr>
             <td class="td-left">System</td>
             <td colspan="2" class="td-right">

                  	<div class="bt-flabels__wrapper">
                        <select name="system" class="tarea-100" id="system" data-parsley-required>
                          ...
                          while($row_system = mysqli_fetch_array($sql_system)){
                              ...
                              if($row_Recordset5['SystemId'] == $row_system['Id']){

                                  $selected = ' selected="selected"';
                              }

                              echo '<option value="'. $row_system['Id'] . '"' . $selected .'>'. $row_system['Name'] .'</option>';
                          }
                          echo '</select>';
           ...
           <tr>
             ...
             <td class="td-right">
               <div class="bt-flabels__wrapper">
                 <input name="att" type="text" class="tarea-100" id="att" value="<?php echo $row_Recordset5['Attention']; ?>" autocomplete="off" data-parsley-required />
                 <span class="bt-flabels__error-desc-dd">Required</span>
               </div>
             </td>
             ...
             <td colspan="2" class="td-right">
               <div class="bt-flabels__wrapper">
                 <input name="fmc" type="text" class="tarea-100" id="fmc" style="cursor:text" value="<?php fmc($con, $_GET['Id']); ?>"  autocomplete="off" data-parsley-required />
                 <span class="bt-flabels__error-desc-dd">Required</span>
               </div>
             </td>
           </tr>
...
-->       <!-- End Requested Completion Date -->
         ...
         <table width="100%" border="0" align="center" cellpadding="0" cellspacing="1">
           ...
           <tr>
             <td colspan="7" class="td-right">
               <div class="bt-flabels__wrapper">
                 <textarea name="notes" rows="5" class="tarea-100" id="notes" autocomplete="off" data-parsley-required><?php echo $row_qs['Notes']; ?></textarea>
                 <span class="bt-flabels__error-desc-dd">Required</span>
               </div>
             </td>
           </tr>
       ...
       <div id="list-border" style="margin-top:15px">
         <table width="100%" border="0" align="center" cellpadding="0" cellspacing="1">
           ...
           <tr>
			 ...
			 if(mysqli_num_rows($query_note_comments) == 0){

				 $required = 'autocomplete="off" data-parsley-required';
			 }

			 ?>

               <div class="bt-flabels__wrapper">
                   <textarea name="internal_notes" rows="3" class="tarea-100" id="internal_notes" <?php echo $required; ?>></textarea>
                   <span class="bt-flabels__error-desc-dd">Required</span>
               </div>
             </td>
           </tr>
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="1" bordercolor="#FFFFFF">
              ...
              <tr>
                <td>
                  ...
                  <div id="qs-thumbs" style="float:left; margin:15px;">
                    <table border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        <td>
                        <?php

						if(file_exists('../photos/'. $row_thumbs['Image']) && !file_exists('../photos/thumbnails/'. $row_thumbs['Image'])){

							createThumbs('../photos/','../photos/thumbnails/',100,$row_thumbs['Image']);
						}

						?>
                          <a href="../photos/<?php echo $row_thumbs['Image']; ?>" class="look_inside" onclick="return hs.expand(this, {captionId: 'caption1'})">
                            <img src="../photos/thumbnails/<?php echo $row_thumbs['Image']; ?>" width="100">
                          </a>
                        </td>
       ...
       <?php if(mysqli_num_rows($query_note_comments) == 0){ ?>
           <div style="margin-top:15px;">
             <table border="0" align="right" cellpadding="0" cellspacing="0">
              <tr>
                <td><input name="save-top" type="submit" class="btn-new" id="save-top" value="Next"></td>
              </tr>
             </table>
           </div>
       <?php } ?>
       ...
       <!-- Costing -->
       <?php if(mysqli_num_rows($query_note_comments) >= 1){ ?>
           <div id="list-border" style="margin-top:15px">
             <table width="100%" border="0" align="center" cellpadding="0" cellspacing="1">
               <tr>
                 <td colspan="7" class="td-header-red"><a href="#" class="toggler sm-bar" data-row="Costing">Costing</a></td>
               </tr>
             </table>
           </div>
             ...
             <div class="rowCosting" style="display:<?php echo $display_div; ?>; border-bottom:solid 2px #d60031; padding-bottom:10px">
             ...
             <table width="100%" border="0" cellpadding="4" cellspacing="1" id="outsource">
               ...
               <?php
                ...
                $query_material2 = mysqli_query($con, "SELECT SUM(Price) AS Total FROM tbl_costing_outsourcing WHERE QuoteNo = '$quoteno' GROUP BY QuoteNo")or die(mysqli_error($db_con));
                $row2 = mysqli_fetch_array($query_material2);
          ...
          ?>
           ...
           <table width="100%" border="0" cellpadding="4" cellspacing="1" id="outsource">
             ...
             <tr>
               ...
               <td class="td-right"><input name="zone" type="text" class="tarea-100-right" id="zone" value="<?php if($row_transport['Trips'] > 0){ zone_charge($row_transport['Trips'], $row_transport['ReturnDistance'], $_SESSION['totals'],$_GET['Id']); } ?>" readonly="readonly" /></td>
               <td align="right" class="td-right"><input name="textfield" type="text" disabled="disabled" class="tarea-100-right" id="textfield" style="font-weight:bold" value="R<?php zone_charge($row_transport['Trips'], $row_transport['ReturnDistance'], $_SESSION['totals'],$_GET['Id']); ?>" /></td>
               <td align="center" class="td-right">&nbsp;</td>
             </tr>
             <tr>
               ...
               <td>
                 <div style="margin-top:15px;">
                   <table border="0" align="right" cellpadding="0" cellspacing="0">
                    <tr>
                      <td>
                        <?php if($costing_total >= 100){ ?>
                          <input name="quote" type="submit" class="btn-new" id="quote" value="Continue To Quote">
                        <?php } ?>
                      </td>
       ...
       <div id="list-border" style="margin-top:15px">
         <table width="100%" border="0" cellspacing="1" cellpadding="0" id="health-q">
           ...
           <tr>
             ...
             <td width="20" class="td-sub-header"><a href="javascript:void(0);" class="btn-add-new" id="add-row-h-q"></a></td>
           </tr>
           <?php
           ...
           <tr>
             ...
             <td width="20" align="center" class="td-right">
             <a name="remove-row" class="remove-row" id="remove-row" onclick="deleteRowHealthQ(this)"></a></td>
           </tr>
           <?php } ?>
         </table>
         ...
         <?php
		  ...
		  $query = mysqli_query($con, "SELECT SUM(Total) AS Total FROM tbl_qs_hes WHERE QuoteNo = '$quoteno'")or die(mysqli_error($con));
		  $row = mysqli_fetch_array($query);
         ...
         <table width="100%" border="0" cellpadding="4" cellspacing="1">
           <tr>
             <td colspan="2" class="td-right">&nbsp;</td>
             <td width="75" align="right" class="td-right"><input name="textfield" type="text" disabled class="tarea-100-right" id="textfield" style="font-weight:bold" value="R<?php echo number_format($row['Total'],2); ?>" /></td>
             <td width="35" align="center" class="td-right">&nbsp;</td>
           </tr>
         </table>
         <!-- Health & Safety -->
         ...
         <table width="100%" border="0" cellspacing="1" cellpadding="0" id="labour-q">
           <tr>
             ...
             <td width="20" class="td-sub-header"><a href="javascript:void(0);" class="btn-add-new" id="add-row-l-q"></a></td>
           </tr>
           <?php
         ...
         <?php
		  ...
		  $query = mysqli_query($con, "SELECT SUM(Total1) AS Total FROM tbl_qs WHERE QuoteNo = '$quoteno' AND Labour = '1'")or die(mysqli_error($con));
		  $row = mysqli_fetch_array($query);
         ...
         <table width="100%" border="0" cellspacing="1" cellpadding="0" id="material-q">
           <tr>
             ...
             <td width="20" class="td-sub-header"><a href="javascript:void(0);" class="btn-add-new" id="add-row-m-q"></a></td>
           </tr>
           <?php
         ...
         <?php
		  ...
		  $query = mysqli_query($con, "SELECT SUM(Total1) AS Total FROM tbl_qs WHERE QuoteNo = '$quoteno' AND Material = '1'")or die(mysqli_error($con));
		  $row = mysqli_fetch_array($query);
         ...
         <?php
		  ...
		  $query = mysqli_query($con, "SELECT SUM(Total) AS Total FROM tbl_qs_equipment WHERE QuoteNo = '$quoteno'")or die(mysqli_error($con));
		  $row = mysqli_fetch_array($query);
         ...
         <table width="100%" border="0" cellspacing="1" cellpadding="0" id="transport-q">
           <tr>
             ...
             <td width="20" class="td-sub-header"><a href="javascript:void(0);" class="btn-add-new" id="add-row-t-q"></a></td>
           </tr>
           <?php
         ...
         <?php
		  ...
		  $query = mysqli_query($con, "SELECT SUM(Total1) AS Total FROM tbl_qs WHERE QuoteNo = '$quoteno' AND Transport = '1'")or die(mysqli_error($con));
		  $row = mysqli_fetch_array($query);
            ...
            <table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
              ...
              <tr>
                <td valign="bottom" class="combo">
                  ...
                  <?php if($_COOKIE['invoiceFromQuote'] == 1){ ?>
                  ...
                  <a href="jc-sla.php?Quote=<?php echo $_GET['Id']; ?>" class="btn-new jobcard">Job Card</a>
                  ...
                  <input name="approval" type="submit" class="btn-new" id="approval" value="Submit For Approval">
                  <?php }  ?>
                </td>
              </tr>
            </table>
