...
if ($_COOKIE['userid'] == 50 || $_COOKIE['userid'] == 142) {
              ...
              <td><table border="0" cellpadding="2" cellspacing="3">
                <tr>
                  ...
                  <select name="company" class="select" id="company">
                    ...
                    <option value="<?php echo $row_companies['Id']; ?>" <?php if($row_companies['Id'] == $_POST['company']){ echo 'selected="selected"'; } ?>><?php echo $row_companies['Name']; ?></option>
                    <?php } ?>
                  </select></td>
                  <td width="130" class="btn-blue-generic"><select name="month" class="select" id="month">
                    <option value="">Month</option>
					<?php for($i=1;$i<=12;$i++){ ?>
                    <option value="<?php echo date('m', mktime(0,0,0,$i,0,0)); ?>" <?php if(date('m', mktime(0,0,0,$i,0,0)) == $_POST['month']){ echo 'selected="selected"'; } ?>><?php echo date('F', mktime(0,0,0,$i,0,0)); ?></option>
                    <?php } ?>
                  </select></td>
                  <td width="130" class="btn-blue-generic"><select name="year" class="select" id="year">
                    <option value="">Year</option>
                    <option value="<?php echo date('Y', strtotime('-1 year')); ?>" <?php if(date('Y', strtotime('-1 year')) == $_POST['year']){ echo 'selected="selected"'; } ?>><?php echo date('Y', strtotime('-1 year')); ?></option>
                    <option value="<?php echo date('Y'); ?>" <?php if(date('Y') == $_POST['year']){ echo 'selected="selected"'; } ?>><?php echo date('Y'); ?></option>
                    <option value="<?php echo date('Y', strtotime('+1 year')); ?>" <?php if(date('Y', strtotime('+1 year')) == $_POST['year']){ echo 'selected="selected"'; } ?>><?php echo date('Y', strtotime('+1 year')); ?></option>
                  </select></td>
                </tr>
              </table></td>
                ...
                <td>
                  <div id="list-brdr-supprt">
                    <table width="100%" border="0" cellpadding="0" cellspacing="1">
                      ...
                      <?php do { ?>
                      <tr class="even">
                        <td align="left" nowrap><a href="inv-calc.php?Id=<?php echo $row_Recordset3['JobId']; ?>&job" class="menu" title="<?php echo $row_Recordset3['JobDescription']; ?>"> &nbsp;<?php echo $row_Recordset3['InvoiceNo']; ?></a>
                        </td>
                        <td align="left" nowrap>&nbsp;<?php echo $row_Recordset3['BatchNo']; ?></td>
                        <td align="left"><a href="inv-calc.php?Id=<?php echo $row_Recordset3['JobId']; ?>&job" class="menu"> &nbsp;<?php echo $row_Recordset3['Name']; ?></a>
                        </td>
                        <td align="left"><a href="inv-calc.php?Id=<?php echo $row_Recordset3['JobId']; ?>&job" class="menu"> &nbsp;<?php echo $row_Recordset3['Name_1']; ?></a>
                        </td>
                        <td width="150" align="left" nowrap><a href="inv-calc.php?Id=<?php echo $row_Recordset3['JobId']; ?>&job" class="menu"> &nbsp;<?php echo $row_Recordset3['InvoiceDate']; ?></a>
                        </td>
                        <td width="150" align="left" nowrap>
                          <select name="status" class="tarea-100" id="status" style="color:#3E7DBD" onChange="MM_jumpMenu('parent',this,0)">
                            ...
                            <?php do { ?>
                            <option value="search.php?Status=<?php echo $row_Recordset5['Id']?>&amp;Id=<?php echo $row_Recordset3['JobId']; ?>" <?php if (!(strcmp($row_Recordset5[ 'Id'], $row_Recordset3[ 'Id_1']))) {echo "selected=\"selected\"";} ?>>
