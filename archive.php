...
$pdf->Cell(190,1);
select_db();
$query = mysql_query("SELECT * FROM tbl_jc WHERE JobId = '$jobid' AND Transport = '1'")or die(mysql_error());
while($row = mysql_fetch_array($query)){
$pdf->Ln(1);
...
$pdf->Ln(1);
$pdf->Cell(190,4,'R'.$summaterial,'','',R);
$pdf->Ln(3);
$pdf->Cell(190,1,'','B','','','0','');
$pdf->Ln(15);
$pdf->Cell(150,1);
...
$query = mysql_query("SELECT InvoiceNo FROM tbl_jc WHERE JobId = '$jobid' ORDER BY Id ASC LIMIT 1")or die(mysql_error());
$row = mysql_fetch_array($query);
...
$query = mysql_query("SELECT * FROM tbl_sent_invoices WHERE JobId = '$jobid'")or die(mysql_error());
...
mysql_query("INSERT INTO tbl_sent_invoices (InvoiceNo,CompanyId,SiteId,JobId,PDF,DateSent) VALUES ('$invoice','$company','$site','$jobid','$document','$date')")or die(mysql_error());
}
