...
$query_debtors = mysqli_query($con, "SELECT * FROM tbl_jc WHERE Status = '12' GROUP BY JobId")or die(mysqli_error($con));
while($row_debtors = mysqli_fetch_array($query_debtors)){

	// Update Sub Totals
	$query_update = mysqli_query($con, "SELECT SUM(Total1) AS Total, SiteId FROM tbl_jc WHERE JobId = '". $row_debtors['JobId'] ."' GROUP BY JobId") or die(mysqli_error($con));
	$row_update = mysqli_fetch_array($query_update);
	
	$query_transport = mysqli_query($con, "SELECT SUM(Total1) AS Total FROM tbl_travel WHERE JobId = '". $row_debtors['JobId'] ."'") or die(mysqli_error($con));
	...
	if($row_sites['VAT'] == 1 || $row_debtors['SiteId'] == NULL){
