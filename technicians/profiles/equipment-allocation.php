...
if(isset($_POST['reset'])){
	...
	header('Location: equipment-allocation.php?ProfileId='. $_GET['ProfileId']);
	exit();
}
...
if(isset($_POST['insert'])){
	...
	dbInsert('tbl_profile_equipment_allocation', $form_data, $con);
...
if(isset($_POST['update'])){
	...
	dbUpdate('tbl_profile_equipment_allocation', $form_data, $where_clause="Id = '". $edit ."'",$con);
	
	header('Location: equipment-allocation.php?ProfileId='. $_GET['ProfileId'] .'&Edit='. $_GET['Edit']);
	exit();
}

if(isset($_GET['Delete'])){
	
	dbDelete('tbl_profile_equipment_allocation', $where_clause="Id = '". $_GET['Delete'] ."'",$con);
	
	header('Location: equipment-allocation.php?ProfileId='. $_GET['ProfileId']);
	exit();
}

$query_form = mysqli_query($con, "SELECT * FROM tbl_profile_equipment_allocation WHERE Id = '$edit'")or die(mysqli_error($con));
$row_form = mysqli_fetch_array($query_form);

$query_current = "
	SELECT
		tbl_profile_equipment_allocation.Id,
		tbl_profile_equipment_allocation.TechId,
		tbl_profile_equipment_allocation.Date,
		tbl_profile_equipment_allocation.Equipment,
		tbl_profile_equipment_allocation.Make,
		tbl_profile_equipment_allocation.SerialNo,
		tbl_profile_equipment_allocation.ConditionAllocated,
		tbl_profile_equipment_allocation.ReturnDate,
		tbl_profile_equipment_allocation.ConditionReturned,
		tbl_profile_equipment_allocation.IssuedBy,
		tbl_technicians.`Name`
	FROM
		tbl_profile_equipment_allocation
	INNER JOIN tbl_technicians ON tbl_profile_equipment_allocation.TechId = tbl_technicians.Id	
	WHERE tbl_profile_equipment_allocation.TechId = '". $_GET['ProfileId'] ."'
	ORDER BY
		tbl_profile_equipment_allocation.Date DESC";
   ...
   <body>
      ...
      <div class="td-bread">
        <ul class="breadcrumb">
          ...
          <li><a href="#">Equipment Allocation</a></li>
          <li></li>
        </ul>
      </div>      
      <!-- End Breadcrumbs -->      
       ...
       <table width="100%" border="0" cellspacing="1" cellpadding="3">
			...
			?>
                    ...
                    <tr class="<?php echo ($ac_sw1++%2==0)?" odd ":"even "; ?> row2000" onMouseOver="this.oldClassName = this.className; this.className='over';" onMouseOut="this.className = this.oldClassName;">
                      ...
                      <td align="center"><a href="equipment-allocation.php?ProfileId=<?php echo $_GET['ProfileId']; ?>&Edit=<?php echo $row_current['Id']; ?>"  class="edit"></a></td>
       ...
       <table width="100%" border="0" cellspacing="1" cellpadding="3">
			...
			?>
                    ...
                    <tr class="<?php echo ($ac_sw1++%2==0)?" odd ":"even "; ?> row1000" onMouseOver="this.oldClassName = this.className; this.className='over';" onMouseOut="this.className = this.oldClassName;">
                      ...
                      <td align="center"><a href="equipment-allocation.php?ProfileId=<?php echo $_GET['ProfileId']; ?>&Edit=<?php echo $row_list['Id']; ?>"  class="edit"></a></td>
                      <td align="center"><a href=""  class="toggler expand" data-prod-cat="<?php echo $i; ?>"></a></td>
                    </tr>
                    <tr class="row<?php echo $i; ?>" style="display: none">
                      <td colspan="7" align="center" class="td-header">
